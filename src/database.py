...
from src.models import Pair, Coin, CurrentCoin, ScoutHistory, \
    CoinValue, Interval, TradeState, Trade, Base
...
class Database:
    ...
    def update_coin(self, coin: Coin, props: dict):
        session: Session
        with self.db_session() as session:
            obj = session.query(Coin).get(coin.symbol)
    ...
    def set_coins(self, symbols: List[str]):
        ...
        with self.db_session() as session:
            ...
            for symbol in symbols:
                coin = next(
                    (coin for coin in coins if coin.symbol == symbol), None
                )
                if coin is None:
        ...
        # in the database, add a pair to the database
        with self.db_session() as session:
            records, pair_records = [], defaultdict()
            coins: List[Coin] = session.query(Coin).filter(Coin.enabled).all()
            for item in session.query(Pair).all():
                pair_records.setdefault((item.from_coin_id, item.to_coin_id))
            for from_coin in coins:
                for to_coin in coins:
                    if from_coin.symbol != to_coin.symbol:
                        if (
                                from_coin.symbol, to_coin.symbol
                        ) not in pair_records:
                            records.append(Pair(from_coin, to_coin))
            session.bulk_save_objects(records)
    ...
    def get_coin(self, coin: Union[Coin, str], enable=None) -> Coin:
    ...
    def get_current_coin(self) -> Optional[Coin]:
        session: Session
        with self.db_session() as session:
            current_coin = session.query(CurrentCoin).order_by(desc(
                CurrentCoin.datetime
            )).first()
    ...
    def get_pairs_from(self, from_coin: Union[Coin, str], only_enabled=True) -> \
            List[Pair]:
        from_coin = self.get_coin(from_coin)
        session: Session
        with self.db_session() as session:
        ...
        with self.db_session() as session:
            ...
            hourly_entries: List[CoinValue] = (
                session.query(CoinValue).group_by(
                    CoinValue.coin_id, func.strftime("%H", CoinValue.datetime)
                ).all()
            )
            ...
            daily_entries: List[CoinValue] = (
                session.query(CoinValue).group_by(CoinValue.coin_id, func.date(
            ...
            weekly_entries: List[CoinValue] = (
                session.query(CoinValue).group_by(
                    CoinValue.coin_id,
                    func.strftime("%Y-%W", CoinValue.datetime)
                ).all()
            )
            ...
            # The last 28 days worth of hourly entries will be kept,
            # so count(coins) * 672 entries
            ...
            # The last years worth of daily entries will be kept,
            # so count(coins) * 365 entries
    ...
    def migrate_old_state(self):
        """
        For migrating from old dotfile format to SQL db.
        ...
        if os.path.isfile(".current_coin"):
            ...
            _logger.info(
                f".current_coin renamed to .current_coin.old -"
...
class TradeLog:
    def __init__(self, db: Database, from_coin: Coin, to_coin: Coin,
                 selling: bool):
        self.db = db
        session: Session

...
from src.models import Pair, Coin, CoinValue
...
class AutoTrader:
    ...
    def transaction_through_bridge(self, pair: Pair):
        ...
        if can_sell and self.manager.sell_alt(pair.from_coin,
                                              self.config.BRIDGE) is None:
        ...
        with self.db.db_session() as session:
            for pair in session.query(Pair).filter(Pair.ratio.is_(None)).all():
                if not pair.from_coin.enabled or not pair.to_coin.enabled:
                    continue
                _logger.info(
                    f"Initializing {pair.from_coin} vs {pair.to_coin}")
    ...
    def _get_ratios(self, coin: Coin, coin_price):
        ...
        for pair in self.db.get_pairs_from(coin):
            ...
            # Fees
            from_fee = self.manager.get_fee(pair.from_coin, self.config.BRIDGE,
                                            True)
            to_fee = self.manager.get_fee(pair.to_coin, self.config.BRIDGE,
                                          False)
            ...
            if self.config.USE_MARGIN == "yes":
                ratio_dict[pair] = (
                        (1 - transaction_fee) *
                        coin_opt_coin_ratio /
                        pair.ratio - 1 - self.config.SCOUT_MARGIN / 10
                )
            else:
                ratio_dict[pair] = (
                                           coin_opt_coin_ratio -
    ...
    def bridge_scout(self):
        ...
        for coin in self.db.get_coins():
            ...
            if not any(v > 0 for v in ratio_dict.values()):
                # There will only be one coin where all the ratios are negative.
                # When we find it, buy it if we can

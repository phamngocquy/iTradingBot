...
class OrderGuard:
    def __init__(self, pending_orders: Set[Tuple[str, int]],
...
class BinanceStreamManager:
    def __init__(self, cache: BinanceCache, config: Config,
                 binance_client: binance.client.Client):
        self.cache = cache
        self.bw_api_manager = BinanceWebSocketApiManager(
            output_default="UnicornFy", enable_stream_signal_buffer=True,
            exchange=f"binance.{config.BINANCE_TLD}"
        )
        self.bw_api_manager.create_stream(
            ["arr"], ["!miniTicker"], api_key=config.BINANCE_API_KEY,
            api_secret=config.BINANCE_API_SECRET_KEY
        )
        self.bw_api_manager.create_stream(
            ["arr"], ["!userData"], api_key=config.BINANCE_API_KEY,
            api_secret=config.BINANCE_API_SECRET_KEY
        )
    ...
    def _fetch_pending_orders(self):
        ...
        for (symbol, order_id) in pending_orders:
            ...
            while True:
                try:
                    order = self.binance_client.get_order(symbol=symbol,
                                                          orderId=order_id)
            ...
            _logger.info(
                f"Pending order {order_id} for symbol "
                f"{symbol} fetched:\n{fake_report}",
                False)
            self.cache.orders[fake_report["order_id"]] = BinanceOrder(
                fake_report)
    ...
    def _stream_processor(self):
        while True:
            ...
            stream_signal = self.bw_api_manager. \
                pop_stream_signal_from_stream_signal_buffer()
            stream_data = self.bw_api_manager. \
            ...
            if stream_signal is not False:
                ...
                if signal_type == "CONNECT":
                    stream_info = self.bw_api_manager.get_stream_info(
                        stream_id)
                    if "!userData" in stream_info["markets"]:
                        _logger.debug("Connect for userdata arrived", False)
